...
Furthermore, $E_l$ and $F_l$ are reference energies and forces, $E(\mathbf{R}^l)$ and $\mathbf{F}(\mathbf{R}^l)=-\boldsymbol{\nabla}_\mathbf{R} E(\mathbf{R}^l)$ $\mathbf{F}(\mathbf{R}^l) = - \boldsymbol{\nabla}_{\mathbf{R}} E(\mathbf{R}) \big|_{\mathbf{R}=\mathbf{R}^l}$ are the energy and force predictions of our model for configuration $l$ in the minibatch, and $\mathbf{R}^l$ and $N_l$ are the atomic coordinates and the number of atoms in configuration $l$.
